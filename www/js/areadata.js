...
function selectProvince(type, con, isremove) {
    ...
    if (type == "all") {
        ...
        strVarCity += '    <p class="data-title" ><a href="javascript:;" onclick="selectallProvince(this)">全部省份</a></p>';
...
function svae_City() {
    ...
    $(dataCityinput).attr("data-value", val);
    $(dataCityinput).val(Cityname);

    $('.duoxuan').each(function () {
        var cityval=$(this).attr("data-value");
        if(cityval){
            var b=cityval.substr(cityval.length-1);
            if(b =="-"){
                var cval=cityval.substring(0,cityval.length-1);
                arrs= arrs+cval+'-';
            }else {
                arrs= arrs+cityval+'-';
            }
    ...
    var strs=arrs.substring(0,arrs.length-1);
    ary=strs.split('-');
    var nary=ary.sort();
    for(var i=0;i<ary.length-1;i++){
        ...
        if (nary[i]==nary[i+1]){
            ...
            $(dataCityinput).attr("data-value", '');
            $(dataCityinput).val('');
            alert("同一城市不可指定多个运费计算方式；");
            return false;
        }

        var aaa=nary[i].substring(0,2);
        var aa=nary[i].substring(2,6);
        var bbb=nary[k].substring(0,2);
...
function selectallProvince(ths) {
    $('.data-result').html('<em>可选择多项</em>');
    var dataCityxz = __LocalDataCities.category.provinces;
    for (var i in dataCityxz) {
        if(dataCityxz[i]){
            $('.data-result').append('<span class="svae_box aui-titlespan" data-code="' + dataCityxz[i] + '" data-name="' + dataarrary[dataCityxz[i]][0] + '" onclick="removespan_area(this)">' + dataarrary[dataCityxz[i]][0] + '<i>×</i></span>');
        }
    }
    $('.clearfix').children('li').each(function () {
        $(this).children('a').addClass('d-item-active');
    })
}